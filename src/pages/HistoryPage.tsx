 ...
 * fed by the shared CollectionDataProvider. Collection-load failure and
 * listening-events-load failure are surfaced independently (Milestone 8):
 * neither hides the other. Day-grouped sections + artwork thumbnails are
 * Phase D.
 */
export function HistoryPage() {
  ...
  return (
    <div className="vi-page">
      ...
      {/* The collection must load first to resolve record titles. */}
      ...
      {status === 'ready' ? (
        <>
          {/* A listening-events failure is shown on its own; the collection
              stayed available. */}
          {eventsStatus === 'error' ? (
            <ErrorState
          ...
          {eventsStatus === 'ready' && ordered.length === 0 ? (
            <EmptyState
          ...
          {eventsStatus === 'ready' && ordered.length > 0 ? (
            ...
            >
              {ordered.map((event) => (
                ...
                >
                  <span>
                    {titleById.get(event.collection_item_id) ??
