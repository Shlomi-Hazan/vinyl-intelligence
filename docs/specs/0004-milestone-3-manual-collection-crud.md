...
- The user can edit permitted manually entered release metadata.
- The user can remove one owned collection item.
- Removing a collection item does not unexpectedly delete shared release
  metadata or another user's ownership.
...
The user may edit the release metadata for manual releases they are allowed to
control. Editing release metadata must not allow one user to change metadata
that another user's collection item references.
...
Provider identifiers, source/provenance fields, timestamps, ownership IDs, and
future catalog fields must not be client-editable.
...
| `provider text` | Nullable, must remain null for Milestone 3 browser manual inserts | Reserved for Milestone 4. |
| `provider_release_id text` | Nullable, must remain null for Milestone 3 browser manual inserts | Release-level provider ID when later available. |
| `provider_master_id text` | Nullable, must remain null for Milestone 3 browser manual inserts | Master/release-group ID when later available. |
...
| `source text` | Not null, default/check `manual` for Milestone 3 | Future source values belong to later migrations. |
...
Milestone 3 needs `releases` to exist before catalog integration, but allowing
all authenticated users to update shared release rows would create a cross-user
...
editing is constrained to the creator and safe reference conditions.

- Pros: preserves the shared table shape needed later, supports manual CRUD now,
  and prevents arbitrary cross-user edits.
- Cons: requires careful RLS policies and later catalog normalization/relinking.
...
- Users can select releases they created or releases referenced by their own
  `collection_items`.
- Users can update manual release metadata only when:
  - `created_by = auth.uid()`,
  - `source = 'manual'`,
  - and no other user's collection item references the same release.
...
Same user adds the exact same release twice:

- Allow it.
- A collection item represents a physical owned copy/entry.
- There should be no unique constraint on `(user_id, release_id)`.
- The UI may optionally show a duplicate warning later, but Milestone 3 should
  not block duplicates silently.
...
| `authenticated` | `releases` | Allow only created/referenced releases | Allow manual release insert only | Allow only safe manual metadata columns on own unshared manual releases | Deny |
| `authenticated` | `collection_items` | Allow own items only | Allow own manual item referencing an allowed release | Deny for Milestone 3 | Allow own items only |
...
- `releases` SELECT: current user created the release, or the release is
  referenced by one of the current user's collection items.
- `releases` INSERT: current user creates a manual row for themselves, with no
  provider identifiers.
- `releases` UPDATE: current user created the manual row and no other user owns
  a collection item pointing at it.
- `collection_items` SELECT: `user_id = auth.uid()`.
- `collection_items` INSERT: `user_id = auth.uid()` and `release_id` references
  a release the current user is allowed to use for manual add.
...
- If release insert succeeds but collection item insert fails, the client
  service should make a best-effort cleanup attempt only for the just-created
  unreferenced manual release if such cleanup is safely permitted by the final
  SQL.
- If cleanup is not permitted, leaving an orphan manual release is acceptable
  because it is not collection ownership and should not be visible in the
  collection list.

Do not introduce a privileged Netlify Function for this. A transactional RPC can
be reconsidered if human review decides orphan manual release rows are
unacceptable, but a `SECURITY DEFINER` helper should not be added just for
convenience.
...
- Provider identifier fields stay null for manual browser rows.
...
- User A cannot edit a release if another user's collection item references it.
...
- Nullable provider identifier fields and the `releases` table allow later
  provider-backed release rows.
- Manual rows can later be matched/relinked to canonical catalog rows after the
  documented API spike.
...
- Approve or revise the recommended manual release ownership/edit model
  (creator-owned manual rows in a shared `releases` table, not globally editable
  shared metadata).
...
- Approve ordinary browser Supabase inserts with RLS and best-effort cleanup
  rather than adding a transactional RPC/helper in Milestone 3.
...
- Approve the proposed RLS/privilege matrix before SQL implementation.
