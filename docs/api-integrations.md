...
As-built section last updated: 2026-09-16 (Discover & MusicBrainz Navigation
Enhancement). The verification / spike material below is preserved as the
selection rationale.
...
Five external services, all reached only through Netlify Functions where a
secret or privileged access is involved. Browser-visible calls are limited to
Cover Art Archive image URLs, a persisted Discogs provider image URL, and
the browser Supabase client (publishable key, RLS authoritative) - none of
these three require a secret in the browser.

| Service | Used for | Auth | Where it runs | Notes |
| --- | --- | --- | --- | --- |
| **MusicBrainz** (`https://musicbrainz.org/ws/2/`) | release search in three explicit modes (All/Artist/Album, field-scoped and Lucene-escaped server-side) with bounded 5-per-page / 20-per-query pagination; an exact-release-by-ID lookup (shared by catalog-add's confirmation step and Discover's paste-a-release-URL flow, `GET /api/catalog/search?releaseId=`); best-effort release-group genre tags on add | `MUSICBRAINZ_USER_AGENT` header, no key | `netlify/functions/_shared/catalog-handlers.mts` (`/api/catalog/search`, `/api/catalog/add`) | selected as the primary/default catalog provider over Discogs (`docs/decisions/0002`); Discogs was later added as an explicit, secondary, user-triggered provider (`docs/decisions/0008`, `docs/specs/0018`) rather than reopening that choice. Paced to <= 1 req/s; response shape **and** pagination metadata (`count`/`offset`) validated before use; timeouts + bounded errors; the exact-lookup path never fetches a client-supplied URL — only a client-validated, server-re-validated MBID crosses the boundary (`docs/specs/0017-discover-musicbrainz-navigation-enhancement.md` §15) |
| **Discogs** (`https://api.discogs.com/`) | secondary, explicit, user-triggered release search (`type=release`, at most 5 results, no pagination); an exact-release-by-ID lookup (`GET /releases/{id}`), used by both the persisting add path and Discover's exact-Discogs-URL import (`docs/specs/0019`); Vinyl-only eligibility enforced server-side | `DISCOGS_TOKEN` (`Authorization: Discogs token=…`, server-only) + `DISCOGS_USER_AGENT` | `netlify/functions/_shared/catalog-handlers.mts` (`/api/catalog/search?provider=discogs`, `/api/catalog/add`); also revalidated from `curator-handlers.mts`'s `loadOwnedCollection` | Never automatic - MusicBrainz remains default/primary. Paced independently from MusicBrainz (`docs/specs/0018` §12/§17). Six-hour content-freshness boundary enforced client- and server-side (`provider_fetched_at`). A persisted `provider_image_url` (string only, never image bytes) is the one Discogs field the browser ever loads directly, via a plain `<img src>` once the server has resolved it (`docs/specs/0019` §7) - `DISCOGS_TOKEN` itself never reaches the browser. |
| **Cover Art Archive** (`https://coverartarchive.org/release[-group]/{mbid}/front-{size}`) | display-time album artwork for a MusicBrainz-backed release | none (public) | browser `<img src>` built client-side in `src/media/AlbumArtwork.tsx` | no key, no proxy, no persisted URL, no `releases.cover_url`; advances to the next tier on `<img>` error, never loops; a Discogs id is never sent here |
...
Not integrated: Discogs OAuth, Marketplace data, or account sync; any
preview/streaming provider; any analytics service.
